...
/// Run betterleaks on multiple paths and return merged, deduplicated findings.
/// Returns an error if betterleaks execution fails on any path.
pub fn run_betterleaks_on_paths(paths: &[&str], tool_path: &str) -> Result<Vec<String>> {
    let mut all_files = std::collections::HashSet::new();
    for path in paths {
        let files = run_betterleaks(path, tool_path)
            .with_context(|| format!("secret detection failed on {path}"))?;
