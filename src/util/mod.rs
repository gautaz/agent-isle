...
pub fn detect_symlink_mode(args: &[String]) -> (Option<String>, Vec<String>) {
    let exec_name = args
        .first()
        .map(PathBuf::from)
    ...
    if is_symlink {
        (Some(exec_name), args.get(1..).unwrap_or(&[]).to_vec())
    } else {
        (None, vec![])
    }
}
...
pub fn xdg_config_home() -> String {
    env::var("XDG_CONFIG_HOME").unwrap_or_else(|_| {
        let home = home_dir().unwrap_or_else(|_| {
            tracing::warn!("$HOME is not set; falling back to /tmp for XDG_CONFIG_HOME");
            "/tmp".to_string()
        });
        format!("{home}/.config")
    })
