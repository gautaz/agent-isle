...
fn test_config_load_yaml() {
    ...
    assert_eq!(
        cfg.mounts[0],
        agent_isle::config::MountConfig::new("/common/ro/path")
    );
    ...
    assert_eq!(
        opencode.mounts,
        vec![agent_isle::config::MountConfig::new("/custom/path")]
    );
    assert_eq!(
...
fn test_config_merge() {
    ...
    base.mounts = vec![agent_isle::config::MountConfig::new("/base/common/ro")];
    base.agents.insert(
        "opencode".to_string(),
        agent_isle::config::AgentConfig {
            binary: "/usr/bin/opencode".to_string(),
            mounts: vec![agent_isle::config::MountConfig::new("/base/path")],
            ..Default::default()
        },
    );

    let mut override_cfg = agent_isle::config::default();
    override_cfg.mounts = vec![agent_isle::config::MountConfig::new("/override/common/ro")];
    override_cfg.agents.insert(
        "opencode".to_string(),
        agent_isle::config::AgentConfig {
            binary: "/usr/bin/override".to_string(),
            mounts: vec![agent_isle::config::MountConfig::new("/override/path")],
            ..Default::default()
        },
    );
