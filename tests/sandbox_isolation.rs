...
#[test]
fn test_sandbox_is_lightweight_op_no_lightweight_config() {
    let agent = agent_no_lightweight();
    // Default config: lightweight_args is empty, returns false.
    assert!(!agent.is_lightweight_op(&["--help".to_string()]));
}
...
#[test]
fn test_sandbox_build_args_podman_proxy_bind() {
    ...
    assert!(bind_idx.is_some(), "podman proxy socket should be bound");
}
